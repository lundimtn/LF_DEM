                  ...
                  std::size_t np)
{
	...
	setPeriodicBoundaryConditions(rhomb_);
	boxMap.resize(np);
...
bool BoxSet::setPeriodicBoundaryConditions(const PBC::RhomboidLattice &rhomb_)
{
	rhomb = rhomb_;
	rhomb_unit_normals = rhomb.getSideNormals();
	rhomb_depths = rhomb.getDepths();
	auto new_box_nb = computeBoxNumber();
	for (unsigned i=0; i<new_box_nb.size(); i++) {
		if(new_box_nb[i] != box_nb[i]) {
			box_nb = new_box_nb;
			setupBoxes();  // trigger rebox
			return true;
		}
	}
	return false;
...
{
	auto box_edges = rhomb.getEdges();
	for (unsigned i=0; i<box_edges.size(); i++) {
		box_edges[i] /= box_nb[i];
	}

	for (unsigned i=0; i<box_nb[0]; i++) {
		for (unsigned j=0; j<box_nb[1]; j++) {
			for (unsigned k=0; k<box_nb[2]; k++) {
				vec3d pos = (i+0.5)*box_edges[0]
							+ (j+0.5)*box_edges[1]
							+ (k+0.5)*box_edges[2];
				...
				for (const auto& a : m10p1) {
					...
					for (const auto& b : m10p1) {
						...
						for (const auto& c : m10p1) {
							delta += c*box_edges[2];
							bx.addNeighborBox(whichBox(rhomb.primaryCellImage(pos+delta)));
						}
					}
				}
			}
...
const vector<unsigned>& BoxSet::neighborhood(unsigned i) const
{
